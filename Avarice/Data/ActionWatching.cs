    ...
    public static class ActionWatching
    {
        ...
        private static void ReceiveActionEffectDetour(ulong sourceObjectId, nint sourceActor, nint position, nint effectHeader, nint effectArray, nint effectTrail)
        {
              ...
              sourceObjectId == Svc.ClientState.LocalPlayer.GameObjectId)
            {
                ...
                _ = ActionSheet.TryGetValue(header.ActionId, out var sheet);
                if(sheet != null)
                {
        ...
        {
            return ActionSheet.TryGetValue(id, out var action) && action.ClassJobCategory is not null ? action.ClassJobLevel : 255;
        }

        public static float GetActionCastTime(uint id)
        {
            return ActionSheet.TryGetValue(id, out var action) ? action.Cast100ms / (float)10 : 0;
        }

        public static int GetActionRange(uint id)
        {
            return ActionSheet.TryGetValue(id, out var action) ? action.Range : -2; // 0 & -1 are valid numbers. -2 is our failure code for InActionRange
        }

        public static int GetActionEffectRange(uint id)
        {
            return ActionSheet.TryGetValue(id, out var action) ? action.EffectRange : -1;
        }

        public static int GetTraitLevel(uint id)
        {
            return TraitSheet.TryGetValue(id, out var trait) ? trait.Level : 255;
        }

        public static string GetActionName(uint id)
        {
            return ActionSheet.TryGetValue(id, out var action) ? (string)action.Name : "UNKNOWN ABILITY";
        }
        ...
        {
            return StatusSheet.TryGetValue(id, out var status) ? (string)status.Name : "Unknown Status";
        }

        public static List<uint> GetStatusesByName(string status)
        {
            return statusCache.TryGetValue(status, out var list)
              ? list
        ...
        {
            return !ActionSheet.TryGetValue(id, out var action)
