...
internal unsafe class Canvas : Window
{
	...
	public override void Draw()
	{
		...
		if (P.currentProfile.CompassEnable && IsConditionMatching(P.currentProfile.CompassCondition))
		{
			...
			{
				Vector2 size = ImGui.CalcTextSize(l);
			...
			{
				if (Svc.GameGui.WorldToScreen(LP.Position with { Z = LP.Position.Z - Prof.CompassDistance }, out Vector2 pos))
				{
			...
			{
				if (Svc.GameGui.WorldToScreen(LP.Position with { X = LP.Position.X - Prof.CompassDistance }, out Vector2 pos))
				{
		...
		{
			foreach (Dalamud.Game.ClientState.Party.IPartyMember x in Svc.Party)
			{
		...
		{
			foreach (IGameObject x in Svc.Objects)
			{
