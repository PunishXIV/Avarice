...
public unsafe class Avarice : IDalamudPlugin
{
	...
	{
		Dalamud.Game.ClientState.Objects.Types.IGameObject obj = Svc.Objects.CreateObjectReference(arg);
		...
		if (flag == ConditionFlag.InCombat)
		{
			Safe(delegate
			{
				...
				else
				{
					...
					{
						int total = currentProfile.CurrentEncounterStats.Hits + currentProfile.CurrentEncounterStats.Missed;
						int success = (int)(100f * currentProfile.CurrentEncounterStats.Hits / total);
	...
	{
		if (P.config.JobProfiles.TryGetValue(job, out string guid))
		{
			if (P.config.Profiles.TryGetFirst(x => x.GUID == guid, out Profile profile))
			{
				return profile;
			}
		}
	...
	private void Tick(object framework)
	{
		...
		{
			uint newJob = Svc.ClientState.LocalPlayer.ClassJob.Id;
			if (newJob != Job)
			{
				PluginLog.Debug($"Job changed from {Job} to {newJob}");
				Profile newJobProfile = GetProfileForJob(newJob);
