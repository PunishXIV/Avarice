...
internal unsafe partial class ConfigWindow : Window
{
	...
	private void Debug()
	{
		...
		}
		{
			...
			_ = ImGui.InputInt("Action override test", ref ActionOverride);
			...
			ImGuiEx.Text($"Current action override: {(Svc.PluginInterface.TryGetData<List<uint>>("Avarice.ActionOverride", out List<uint> data) ? data[0] : 0)}");
			ImGuiEx.Text($"Combo: {P.memory.LastComboMove}");
			foreach (Dalamud.Game.ClientState.Statuses.Status x in Svc.ClientState.LocalPlayer?.StatusList)
			{
	...
	{
		CardinalDirection result = Svc.PluginInterface.GetIpcSubscriber<IntPtr, CardinalDirection>("Avarice.CardinalDirection").InvokeFunc(Svc.Targets.Target?.Address ?? IntPtr.Zero);
	...
	internal static void DrawUnfilledSettings(string id, ref Brush b, bool displayCondition = true)
	{
		if (displayCondition)
		{
			...
			_ = ImGuiEx.EnumCombo($"##b{id}", ref b.DisplayCondition);
		...
		_ = ImGui.DragFloat($"##c{id}", ref b.Thickness, 0.1f, 0f, 10f);
		ImGui.SameLine();
		ImGuiEx.Text($"  Color:");
		ImGui.SameLine();
		_ = ImGui.ColorEdit4($"##a{id}", ref b.Color, ImGuiColorEditFlags.NoInputs);
	}
	...
	internal static void DrawUnfilledMultiSettings(string id, ref Brush b, ref Vector4 south, ref Vector4 east, ref Vector4 west, ref bool lines, ref bool makeSameColor)
	{
		...
		ImGui.SameLine();
		_ = ImGui.Checkbox($"Colour match borders?##{id}", ref makeSameColor);
		...
		if (!makeSameColor)
		{
			...
			ImGuiEx.Text($"            Rear:");
			ImGui.SameLine();
			_ = ImGui.ColorEdit4($"##a{id}s", ref south, ImGuiColorEditFlags.NoInputs);
			ImGuiEx.Text($"            Left Flank:");
			ImGui.SameLine();
			_ = ImGui.ColorEdit4($"##a{id}e", ref east, ImGuiColorEditFlags.NoInputs);
			ImGuiEx.Text($"            Right Flank:");
			ImGui.SameLine();
			_ = ImGui.ColorEdit4($"##a{id}w", ref west, ImGuiColorEditFlags.NoInputs);
		}
		ImGuiEx.InvisibleButton(11);
		ImGui.SameLine();
		_ = ImGui.Checkbox($"Display zoning separator lines?##{id}", ref lines);
		ImGuiEx.InvisibleButton(11);
		ImGui.SameLine();
